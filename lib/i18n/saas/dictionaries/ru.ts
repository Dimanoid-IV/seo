...
export const saasDictionary: SaasDictionary = {
  nav: {
    ...
    publicationCalendar: "Календарь публикаций",
    socialPosts: "Соцсети",
    reports: "Отчёты",
    timeline: "История",
  ...
  dashboard: {
    ...
    setupHelp: {
      title: "Нужна помощь с настройкой RankBoost?",
      description:
        "Если не хочется разбираться с WordPress, custom-сайтом, Search Console или автопубликацией, отправьте запрос — мы поможем подключить всё безопасно.",
      cta: "Запросить помощь",
    },
    status: {
      setup: {
  ...
  integrations: {
    ...
    githubPr: {
      ...
      description:
        "Для custom/static сайтов RankBoost может добавить готовую статью как Markdown-файл и открыть pull request вместо прямого изменения production.",
      owner: "Owner",
      repo: "Repository",
      baseBranch: "Base branch",
      contentPath: "Папка контента",
      token: "GitHub token",
      tokenPlaceholder:
        "Fine-grained token с доступом к contents и pull requests",
      tokenNote:
        "Token шифруется и больше не показывается. Используйте fine-grained token для одного repository с правами Contents и Pull requests.",
      testAndSave: "Проверить и сохранить",
      disconnect: "Отключить",
      connected: (repo, path) => `Подключено: ${repo} → ${path}`,
      connectedFallback: "GitHub подключён.",
      disconnected: "GitHub PR публикация отключена.",
      connectFailed: "Не удалось подключить GitHub.",
      connectNetworkError: "Сетевая ошибка при подключении GitHub.",
    ...
    shopify: {
      title: "Публикация в Shopify Blog",
      description:
        "Подключите Shopify Admin API token и blogId. RankBoost будет создавать blog articles из статей, прошедших quality gate.",
      shopDomain: "Shopify domain",
      blogId: "Blog ID",
      token: "Admin API token",
      tokenPlaceholder: "Token с правом write_content",
      authorName: "Автор статьи",
      tokenNote:
        "Token шифруется и больше не показывается. Используйте domain вида your-store.myshopify.com и blogId из Shopify Admin GraphQL.",
      testAndSave: "Проверить и сохранить",
      disconnect: "Отключить",
      connected: "Shopify подключён. Готовые статьи можно отправлять в блог магазина.",
      disconnected: "Shopify отключён.",
      connectFailed: "Не удалось подключить Shopify.",
      connectNetworkError: "Сетевая ошибка при подключении Shopify.",
      disconnectFailed: "Не удалось отключить Shopify.",
      disconnectNetworkError: "Сетевая ошибка при отключении Shopify.",
    },
    wix: {
      title: "Публикация в Wix Blog",
      description:
        "Подключите Wix Site ID и API key. RankBoost будет создавать draft posts из статей, прошедших quality gate.",
      siteId: "Wix Site ID",
      apiKey: "Wix API key",
      tokenNote:
        "API key шифруется и больше не показывается. Используйте ключ с доступом к Wix Blog draft posts для одного сайта.",
      testAndSave: "Проверить и сохранить",
      disconnect: "Отключить",
      connected: "Wix подключён. Готовые статьи можно отправлять как drafts в Wix Blog.",
      disconnected: "Wix отключён.",
      connectFailed: "Не удалось подключить Wix.",
      connectNetworkError: "Сетевая ошибка при подключении Wix.",
    ...
    ghost: {
      ...
      description:
        "Подключите Ghost Admin API key. RankBoost будет создавать posts из статей, прошедших quality gate.",
      adminUrl: "Ghost URL",
      adminKey: "Admin API key",
      authorSlug: "Author slug",
      tokenNote:
        "Admin key шифруется и больше не показывается. Используйте ключ формата id:secret из Ghost Admin → Integrations.",
      testAndSave: "Проверить и сохранить",
      disconnect: "Отключить",
      connected: "Ghost подключён. Готовые статьи можно отправлять в Ghost.",
      disconnected: "Ghost отключён.",
      connectFailed: "Не удалось подключить Ghost.",
      connectNetworkError: "Сетевая ошибка при подключении Ghost.",
    ...
    customWebsite: {
      title: "Публиковать на своём сайте",
      description:
        "Для сайта не на WordPress нужен один защищённый адрес публикации. Ваш разработчик добавляет его один раз, а дальше RankBoost сможет отправлять готовые статьи кнопкой или по утверждённому месячному плану.",
      copyDeveloperTask: "Скопировать короткую инструкцию",
      copiedDeveloperTask: "Инструкция скопирована",
      endpointStepTitle: "1. Разработчик добавляет адрес",
      endpointStepDescription: "Обычно это /api/rankboost/articles на вашем сайте.",
      copyEndpointExample: "Скопировать пример URL",
      copiedEndpointExample: "URL скопирован",
      testStepTitle: "2. Вы проверяете подключение",
      testStepDescription: "RankBoost отправляет безопасный тест без статьи.",
      publishStepTitle: "3. RankBoost публикует",
      publishStepDescription: "Статья отправляется на сайт кнопкой или автопилотом.",
      addWebsiteFirst: "Добавьте сайт, чтобы настроить публикацию.",
      endpointRequired: "Укажите URL для публикации.",
      testFailed: "Проверка не удалась.",
      testNetworkError: "Сетевая ошибка при проверке сайта.",
      testSuccess:
        "Сайт подключён. Откройте готовую статью и нажмите «Опубликовать на сайте».",
      disconnectSuccess: "Публикация на свой сайт отключена. Secret и URL удалены.",
      disconnectFailed: "Не удалось отключить.",
      disconnectNetworkError: "Сетевая ошибка при отключении.",
      connectedTitle: "Публикация на сайт подключена",
      connectedDescription:
        "Всё готово: откройте готовую статью и нажмите «Опубликовать на сайте». RankBoost отправит материал прямо в подключённый блог. Полный URL и secret не отображаются.",
      secretSaved: "HMAC secret сохранён и не показывается.",
      openReadyArticles: "Открыть готовые статьи",
      replaceEndpoint: "Заменить адрес",
      disconnect: "Отключить",
      currentlyConnected: (host) =>
        `Сейчас подключено: ${host}. Введите новый URL для повторной проверки.`,
      endpointUrlLabel: "URL для публикации",
      sharedSecretLabel: "Shared secret для подписи",
      keepSecretPlaceholder: "Оставьте пустым, чтобы сохранить прежний secret",
      secretPlaceholder: "secret-для-подписи",
      secretHiddenNote: "Secret больше не показывается после сохранения.",
      testAndSave: "Проверить и сохранить",
      cancel: "Отмена",
      errorCopyInstruction: "Не удалось скопировать инструкцию.",
      errorCopyUrl: "Не удалось скопировать URL.",
      technicalDetails: "Что передать разработчику",
      technicalIntro:
        "Endpoint должен принимать POST JSON и отвечать HTTP 2xx. Тест шлёт event: \"rankboost.test\". Реальная отправка — article.ready или site.fix.ready только после явного действия пользователя или после включения Auto-publish в подтверждённом месячном плане. URL и secret не логируются.",
      nextjsExample: "Готовый пример для Next.js / Vercel",
      payloadExample: "Payload пример",
      headers: "Headers",
      successResponse: "Успешный ответ",
      hmacHeader: "HMAC заголовок X-RankBoost-Signature",
      fallbackHint:
        "Если сайт ещё не подключён — используйте универсальный пакет HTML/Markdown/email в карточке статьи.",
      copyInstruction: "Скопировать инструкцию разработчику",
  ...
  reviewPage: {
    ...
    openTasks: "Открыть задачи",
    safetyNote:
      "Проверьте готовые материалы. Если сайт подключён, статью можно отправить одной кнопкой; автопубликация работает только в подтверждённом месячном плане.",
    fromTask: "Из задачи",
    updatedLabel: "Обновлено",
    approve: "Одобрить",
    applyToSite: "Применить на сайте",
    edit: "Редактировать",
    reject: "Отклонить",
    markDone: "Отметить выполненной",
    ...
    articleReadyCustomHint:
      "Сайт подключён. Проверьте текст и нажмите «Опубликовать на сайте», чтобы отправить статью прямо в блог.",
    ...
    whatsNextCustomSteps: [
      "Проверьте статью",
      "При необходимости отредактируйте",
      "Нажмите «Опубликовать на сайте»",
      "RankBoost отправит статью в подключённый блог",
    ],
    customPackageHint:
  ...
  publicationCalendar: {
    ...
    subtitle:
      "Все статьи подтверждённого плана по датам. Нажмите на готовую статью, чтобы открыть и проверить её.",
    loading: "Загружаем календарь публикаций…",
    loadFailed: "Не удалось загрузить календарь публикаций",
    openPlan: "Открыть месячный план",
    scheduled: "Запланировано",
    ready: "Статей готово",
    published: "Опубликовано",
    nextPublication: "Следующая публикация",
    noDate: "Пока нет",
    today: "Сегодня",
    previousMonth: "Предыдущий месяц",
    nextMonth: "Следующий месяц",
    approvedHint: "План подтверждён. Автопилот работает по этим датам.",
    draftHint: "Подтвердите месячный план, чтобы зафиксировать расписание.",
    emptyTitle: "На этот месяц публикаций пока нет",
    emptyDescription:
      "Откройте месячный план, выберите темы и подтвердите расписание. После этого статьи появятся здесь по датам.",
    preparing: "Статья готовится",
    preparingDescription:
      "Тема уже стоит в расписании, но текст статьи ещё готовится. Когда черновик будет создан, он откроется прямо из календаря.",
