...
assert.match(CUSTOM_FIX_WEBHOOK_PAYLOAD_EXAMPLE, /"event": "site\.fix\.ready"/);
assert.match(CUSTOM_FIX_WEBHOOK_SUCCESS_RESPONSE_EXAMPLE, /"applied": true/);

const brief = buildCustomWebhookDeveloperBrief();
assert.match(brief, /rankboost\.test/);
assert.match(brief, /article\.ready/);
assert.match(brief, /site\.fix\.ready/);
assert.match(brief, /applied: true/);
